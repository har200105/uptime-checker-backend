...
import { createMongoHeartBeat } from '../services/mongo.service';
import { emailSender, locals } from '../utils/helpers';
...
class MongoMonitor {
  ...
  async start(data: IMonitorDocument): Promise<void> {
    ...
    try {
      ...
      const monitorData: IMonitorDocument = await getMonitorById(monitorId!);
      this.mongoDBError(monitorData, error as IMonitorResponse);
    }
  }
