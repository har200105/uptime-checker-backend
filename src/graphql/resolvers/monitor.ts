import { AppContext, IMonitorArgs, IMonitorDocument } from '../../interfaces/monitor.interface';
import { createMonitor, deleteSingleMonitor, getHeartbeats, getMonitorById, getUserActiveMonitors, getUserMonitors, startCreatedMonitors, toggleMonitor, updateSingleMonitor } from '../../services/monitor.service';
...
import { appTimeZone, authenticateGraphQLRoute, resumeMonitors, uptimePercentage } from '../../utils/helpers';
  ...
  Mutation: {
    async createMonitor(_: undefined, args: IMonitorArgs, contextValue: AppContext) {
      ...
      if (body.active && monitor?.active) {
        startCreatedMonitors(monitor, toLower(body.name), body.type as MonitorType);
      }
      return {
        monitors: [monitor]
      };
